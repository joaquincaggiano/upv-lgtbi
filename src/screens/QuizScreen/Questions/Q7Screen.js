...
export default class Q7Screen extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      selected: this.props.route.params?.q7?.selected,
      showModal: false,
    }
  }

  toggleModal = () => this.setState({showModal: !this.state.showModal});
  goBack = () => this.props.navigation.navigate('Q6', {...this.props.route.params,
    q7: {
      selected: this.state.selected,
      correct: this.state.selected ? this.answers[this.state.selected -1].correct : false,
    }
  });
  next = () => this.props.navigation.navigate('Q8', {...this.props.route.params,
    q7: {
      selected: this.state.selected,
      correct: this.state.selected ? this.answers[this.state.selected -1].correct : false,
    }
  ...
  answers = [
    {
      id: 1,
      text: I18n.t('quiz7_a1'),
      correct: true,
    },
    {
      id: 2,
      text: I18n.t('quiz7_a2'),
    },
  ]
      ...
      <Layout
        ...
        <View style={{paddingHorizontal: paddingScreens}}>
          <View style={[styles.box, {backgroundColor: '#3D348B', paddingVertical: 40, paddingHorizontal: 20}]}>
            <Text style={[styles.text, styles.fontWeightBold, {textAlign: 'center', color: '#FFF'}]}>{I18n.t('quiz7_p')}</Text>
          </View>
        ...
        <View style={{flex: 1, justifyContent: 'flex-end', }}>
          <View style={{alignItems: 'flex-end', marginBottom: 12, marginRight: paddingScreens}}>
            <Text style={[styles.text, styles.fontSizeXsmall, styles.fontWeightSemiBold]}>7/10</Text>
          </View>
          <View style={{flexDirection: 'row', width: '100%'}}>
            <View style={{flex: 7, backgroundColor: '#3D348B', height: 10}}></View>
            <View style={{flex: 3, backgroundColor: '#E3E0FF', height: 10}}></View>
          </View>
        </View>
        <Modal
          animationIn={'fadeIn'}
